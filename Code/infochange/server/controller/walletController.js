...
walletController.bizum = async (req, res) => {
    if (!req.session.user) {
        ...
        try {
            ...
            utils.applog("Updating sender wallet...", "TEST");

            if (newBizumerAmount === 0) await senderWallet.destroy();
            else
                await senderWallet.update({
                    quantity: newBizumerAmount.toFixed(8),
                });

            utils.applog("Updated sender wallet!", "TEST");
            ...
            utils.applog("Updating receiver wallet...", "TEST");
            if (receiverWallet) {
                const newAmount =
            ...
            utils.applog("Updated receiver wallet!", "TEST");
...
walletController.trade = async (req, res) => {
    ...
    utils.applog(JSON.stringify(symbolPriceObject), "DEBUG SYMBOL");
    ...
    try {
        if (!req.session.user) {
            ...
            if (!wallet || wallet.quantity >= cart.quantity) {
                ...
                wallet.update({
                    quantity: wallet.quantity - cart.quantity,
                });
