...
function initModels(sequelize) {
  ...
  user.belongsTo(secure_question, { as: "secureQuestion_secure_question", foreignKey: "secureQuestion"});
  secure_question.hasMany(user, { as: "users", foreignKey: "secureQuestion"});
  bizum_history.belongsTo(user, { as: "receiver_user", foreignKey: "receiver"});
  user.hasMany(bizum_history, { as: "bizum_histories", foreignKey: "receiver"});
  bizum_history.belongsTo(user, { as: "sender_user", foreignKey: "sender"});
  user.hasMany(bizum_history, { as: "sender_bizum_histories", foreignKey: "sender"});
  payment_history.belongsTo(user, { as: "user_user", foreignKey: "user"});
  user.hasMany(payment_history, { as: "payment_histories", foreignKey: "user"});
